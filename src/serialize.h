...
uint64_t ReadCompactSize(Stream& is, bool range_check = true)
{
    ...
    else if (chSize == 253)
    {
        ...
        if (nSizeRet < 253) {
            std::cout << "non-canonical ReadCompactSize()" << std::endl;
            exit(1);
        }
    }
    else if (chSize == 254)
    ...
    if (range_check && nSizeRet > MAX_SIZE) {
        std::cout << "ReadCompactSize(): size too large" << std::endl;
        exit(1);
    }
    return nSizeRet;
}
...
I ReadVarInt(Stream& is)
{
    ...
    while(true) {
        ...
        if (n > (std::numeric_limits<I>::max() >> 7)) {
            std::cout << "ReadVarInt(): size too large" << std::endl;
            exit(1);
        }
        n = (n << 7) | (chData & 0x7F);
        if (chData & 0x80) {
...
struct CustomUintFormatter
{
    ...
    {
        if (v < 0 || v > MAX) {
            std::cout << "CustomUintFormatter value out of range" << std::endl;
            exit(1);
        };
        if (BigEndian) {
...
struct CompactSizeFormatter
{
    ...
    void Unser(Stream& s, I& v)
    {
        ...
        if (n < std::numeric_limits<I>::min() || n > std::numeric_limits<I>::max()) {
            std::cout << "CompactSize exceeds limit of type" << std::endl;
            exit(1);
        }
        v = n;
    }
...
struct LimitedStringFormatter
{
    ...
    void Unser(Stream& s, std::string& v)
    {
        ...
        if (size > Limit) {
            std::cout << "String length limit exceeded" << std::endl;
            exit(1);
        }
        v.resize(size);
